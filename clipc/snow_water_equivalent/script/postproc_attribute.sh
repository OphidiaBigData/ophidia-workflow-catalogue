...
ncatted -O -h -a title,global,o,c,"Snow Water Equivalent, monthly aggregate valueSnow Water Equivalent, monthly aggregate value" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 2
fi
ncatted -O -h -a activity,global,o,c,"SWE" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 3
fi
...
ncatted -O -h -a comment,global,o,c,"Snow Water Equivalent, monthly aggregate valueSnow Water Equivalent, monthly aggregate value" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 8
fi
...
ncatted -O -h -a keywords,global,o,c,"SWE" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 12
fi
...
ncatted -O -h -a platform_id,global,o,c,"NIMBUS" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 23
fi
...
ncatted -O -h -a satellite_sensor,global,o,c,"SMMR" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 25
fi
...
ncatted -O -h -a time_coverage_start,global,o,c,"197910" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 28
fi
ncatted -O -h -a time_coverage_end,global,o,c,"200806" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 29
fi
ncatted -O -h -a time_coverage_duration,global,o,c,"345" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 30
fi
