...
ncatted -O -h -a title,global,o,c,"Sea Surface Temperature, ESA CCI OSTIA L4" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 2
fi
ncatted -O -h -a activity,global,o,c,"SST" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 3
fi
ncatted -O -h -a product,global,o,c,"ESA SST CCI OSTIA L4" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 4
fi
...
ncatted -O -h -a comment,global,o,c,"Sea Surface Temperature, ESA CCI OSTIA L4, climatology monthly average and anomaly" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 8
fi
...
ncatted -O -h -a keywords,global,o,c,"Oceans > Ocean Temperature > Sea Surface Temperature" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 12
fi
...
ncatted -O -h -a platform_id,global,o,c,"ERS-<1,2>, Envisat, NOAA-<12,14,15,16,17,18>, MetOpA" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 23
fi
...
ncatted -O -h -a satellite_sensor,global,o,c,"ATSR, AATSR, AVHRR_GAC" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 25
fi
...
ncatted -O -h -a time_coverage_start,global,o,c,"199109" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 28
fi
ncatted -O -h -a time_coverage_end,global,o,c,"201012" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 29
fi
ncatted -O -h -a time_coverage_duration,global,o,c,"232" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 30
fi
ncatted -O -h -a time_coverage_resolution,global,o,c,"mon" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 31
fi
...
ncatted -O -h -a geospatial_bounds,global,o,c,"POLYGON (-90 -180, 90 -180, 90 180, -90 180)" $filepath/$filename
if [ $? -ne 0 ]; then
        exit 34
fi
ncatted -O -h -a geospatial_lat_min,global,o,f,-90 $filepath/$filename
if [ $? -ne 0 ]; then
        exit 35
fi
ncatted -O -h -a geospatial_lat_max,global,o,f,90 $filepath/$filename
if [ $? -ne 0 ]; then
        exit 36
fi
...
ncatted -O -h -a geospatial_lat_resolution,global,o,f,0.05 $filepath/$filename
if [ $? -ne 0 ]; then
        exit 39
fi
ncatted -O -h -a geospatial_lon_resolution,global,o,f,0.05 $filepath/$filename
if [ $? -ne 0 ]; then
        exit 40
fi
